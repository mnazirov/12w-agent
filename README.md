...
| `/motivation` | Настройки мотивации: включение, интервал (от 1 минуты до 48 часов), стиль |
...
- генерацию контекста для AI-мотивации на основе вовлечённости, достижений и точных действий за сегодня.

Как это работает:
- middleware трекает команды/сообщения/callback и отправляет события в MCP;
- APScheduler каждую минуту проверяет, кому нужна мотивация;
- бот формирует промпт с точными флагами `plan_today`/`checkin_today` и `days_since` по ключевым действиям, затем генерирует сообщение через OpenAI;
- факт отправки записывается в MCP для контроля интервала.

Интервал мотивации:
- хранится в `interval_hours` (поддерживаются дробные значения, например `0.0167` = 1 минута);
- задаётся через `/motivation`, включая ручной ввод в минутах;
- допустимый диапазон: от **1 минуты** до **48 часов**.
